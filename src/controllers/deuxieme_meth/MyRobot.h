...
// ─────────────────────────────────────────────────────────────
// Temporary wall-based odometry calibration constants
// ─────────────────────────────────────────────────────────────

#define BACK_WALL_HEADING           3.14159
#define LEFT_FROM_BACK_HEADING -1.5708
#define BACK_RIGHT_CORNER_HEADING   -2.35779



#define WALL_FRONT_THRESHOLD       900.0
#define CORNER_FRONT_THRESHOLD     900.0
#define CALIB_TURN_SPEED           1.2
#define CALIB_DRIVE_SPEED          1.5
#define CALIB_SLOW_SPEED           0.5
#define CORNER_PUSH_STEPS          20
...
private:
    ...
    enum State {
        ID_TURN_TO_BACK_WALL,
        ID_DRIVE_TO_BACK_WALL,
        ID_TURN_RIGHT_FROM_BACK_WALL,
        ID_DRIVE_TO_LEFT_CORNER,
        ID_SET_RIGHT_CORNER_ODOM,
        ID_MEASURE_LIGHT_CALIB,
        ID_CALIB_DONE,

        // Old cases are kept below but disabled in MyRobot.cpp with #if 0.
        ID_FACE_FORWARD,
        ID_MEASURE_LIGHT,
    ...
    double _id_avg_g;
    double _id_avg_b;
    double _id_variance;          // <-- ajout
    double _id_saturation_ratio;  // <-- ajout
    ...
    bool _corner_wall_seen;
    int  _corner_push_steps;

    bool _back_wall_seen;
    int _back_wall_push_steps;
    ...
    bool drive_to_back_wall();
    bool go_to_right_corner();
    bool drive_to_right_corner_after_back_wall();
    bool turn_to_heading_forced_left(double target, double speed);
    bool look_back_right();
    ...
    bool go_to_point(double target_x, double target_y);

    double measure_front_avg(int samples);
    double go_to_point_and_measure_front(double gx, double gy, double final_theta);
    int idmonde4567();
    int idmonde1810();
